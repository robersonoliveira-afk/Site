# Briefing — decks das aulas teóricas · 2026/1 · v3 (T7–T8)
...
| aula-t6.html | pronto | 3f8c4e0 | 28 |
| **aula-t7.html** | **próximo** | — | — |
...
**Template**: copiar integralmente o CSS/JS de `aula-t6.html` (acento terra, correto para T7–T8). Estrutura idêntica: um `<section class="slide" data-act="...">` por slide; navegação por setas/clique/swipe/hash; contador; `@media print` = apostila; colofão com declaração de IA. O guia de edição no topo do arquivo deve ser mantido.

## Design system

Fraunces (títulos) + IBM Plex Sans (corpo) + IBM Plex Mono (números/fórmulas), via Google Fonts. Tokens: `--paper #FAF7F0 · --paper-warm #F4EFE2 · --ink #1A1A17 · --ink-soft #3A3833 · --stone #6B6960 · --olive #3D4A2A · --mustard #B89233 · --terra #A8552E · --rule #D9D3C2`. T7–T8 = movimento **Decidir** = acento **terra** (`--accent:var(--terra); --accent-text:#8A4322; --accent-deep:#93492A; --terra-ink:#3D1E0E`; célula `.now` do mapa: fundo terra, texto paper). Fundo papel, sentence case, sem emoji, sem arroba de boi (usar kg, litros, ha, sacas).
...
1. **Onde estamos** (2–3): capa; mapa do semestre (marcar a semana com `.now` — T7 = S11, T8 = S12); o que o grupo já tem + "hoje destrava".
2. **Conceito** (12–18): um conceito por slide, fórmulas em `.formula`.
3. **Exemplo RS trabalhado** (4–6): caso gaúcho com números reais passo a passo (CEPEA, CONAB, EMATER/RS, IRGA, Campo Futuro/CNA, BCB).
...
- Ganchos AP em cada aula (receita/custo/margem por zona de manejo) — plantados em T2, T3, T5 e T6 (T6: taxa variável = RM = CM por zona; viabilidade do investimento AP ficou para a T8-AP).

## Pendências

- **Locks do git**: ficaram `HEAD.lock` e `index.lock` em `Novo/Site/.git/` que a sessão não conseguiu apagar (OneDrive). O professor remove manualmente antes do próximo commit; se a sessão travar de novo, tentar `rm -f .git/*.lock` e repetir.
...
- Há um diretório `.claude/` não rastreado dentro de `Novo/Site/` — ignorar ou adicionar ao `.gitignore`.

## Materiais e dados já levantados (reutilizar sem nova busca)

- **IRGA custo arroz 2025/26** (publ. jun 2026, usado nas T3 e T6): CT R$ 15.770,36/ha; produtividade 174,79 sc/ha; custo R$ 90,22/sc; CV R$ 11.781,29/ha (74,7%) = R$ 67,40/sc; depreciação R$ 1.245,59/ha; renda dos fatores R$ 2.743,47/ha; preço ref. R$ 61,02/sc. Na T6 virou o caso de MS negativa (PE produção sobre CT = 258,4 sc/ha, acima do teto biológico).
- **Preços jul/2026 (usados na T6)**: soja CEPEA/Paranaguá R$ 141,02/sc (17 jul, maior nível em 6 meses, +5,6% no mês); dólar R$ 5,09 (20 jul, veio de >5,20 no início do mês).
- **Cenários soja T6 (referência didática, base T2)**: 50 ha, 60 sc/ha, R$ 128/sc; COE R$ 4.480/ha, CT R$ 5.760/ha (= 45 sc/ha, na linha dos levantamentos 25/26 do RS); RL = RB × 0,9687 (encargos 3,13%). PE(COE) 35 sc, PE(CT) 45 sc, MS 25%. Pessimista 48 sc × R$ 109, CT +5% → LE −980/ha (−49.000); base +1.680/ha (+84.000); otimista 66 sc × R$ 138 → +3.063/ha (+153.150).
- **Exemplo T5 (leite, região central)**: 30 vacas, 600 L/dia, 219.000 L/ano; por litro: RL 2,40 · COE 2,05 · COT 2,45 · CT 2,75 → MB +0,35 · ML −0,05 · LE −0,35 (referência didática padrão Campo Futuro; atualizar em aula). **Reutilizável na T8-Agropecuária.**
- **Exemplo T4 (trator 75 cv)**: R$ 200.000, residual 20%, 10 anos/10.000 h → R$ 16.000/ano, R$ 16/h (mercado 2026: R$ 160–260 mil). Reutilizável na T8 (payback/VPL/TIR de máquina).
...
- Os pptx/pdf antigos ("Aula N") seguem numeração antiga e **divergem** do plano novo. São insumo, não base. Para a T7: CARTILHA_DIGITAL_CrditoPlanoSafra20252026.pdf (na raiz da pasta).

## Resumo das aulas restantes

### T7 · S11 — Crédito rural e fluxo de caixa (terra)

Objetivos: Plano Safra, enquadramento (Pronaf/Pronamp/demais), custeio × investimento; fluxo de caixa e capital de giro; checklist de documentos (retomar croqui/CAR da S2 — referência cruzada, sem reensinar). Site: Parte IV blocos 1–2 + painel `credito.html` (cada aluno consulta seu município). Exemplo RS: custeio × investimento no painel BCB; juros Pronaf vs. taxa livre. Simulador: **passo 0, seção crédito**. Saída: enquadramento do projeto justificado. Material de apoio na pasta: CARTILHA_DIGITAL_CrditoPlanoSafra20252026.pdf — **conferir se saiu Plano Safra 2026/27 (buscar; em jul/2026 já deve haver anúncio)**. Gancho plantado na T6: o cenário pessimista dimensiona o crédito e a proteção.

### T8 · S12 — Módulos aplicados + investimento (terra) — DUAS VARIANTES

`aula-t8-agropecuaria.html` e `aula-t8-ap.html`, parte comum idêntica. Comum: perenes (implantação, formação, análise plurianual); payback, VPL e TIR. Variante Agropecuária: UA e lotação, custos no confinamento, leite (calculadora `rebanho.html`; exemplo T5 reutilizável). Variante AP: tecnologia como decisão econômica; viabilidade do investimento em AP (monitor de colheita se paga em quantos ha?) — gancho plantado na T6 ("se o equipamento se paga é a pergunta da T8"). Site: Parte V conforme turma + `vinhedo.html`. Simulador: **passos 5–6**. Saída: perenes com passo 5 fechado.

## Processo

1. Ler o template (`aula-t6.html`) e os blocos do site da aula antes de montar; material antigo é insumo, não base.
